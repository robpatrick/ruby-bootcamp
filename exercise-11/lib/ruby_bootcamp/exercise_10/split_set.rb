  ...
  module Exercise10
    class SplitSet < Hash
      ...
      def add(id:, value:, percentage:)
        fail InvalidSplitException, 'splits must be a Fixnum' unless percentage.is_a?(Fixnum)
        lower = values.last ? values.last.upper : 0
        upper = lower + percentage

        fail InvalidSplitException, 'total split percentages can not be greater than 100%' if upper > 100
